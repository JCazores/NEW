...
<title>VAPT Security Report — {{ $scan->target }}</title>
</head>
  ...
  <table width="620" cellpadding="0" cellspacing="0" style="max-width:620px; width:100%;">
    ...
    <tr>
      <td style="background:linear-gradient(135deg,#0f172a,#1e293b); border-radius:16px 16px 0 0; padding:36px 40px;">
          ...
          <tr>
            <td>
              <table cellpadding="0" cellspacing="0">
                <tr>
                  <td style="background:rgba(6,182,212,0.15); border:1.5px solid rgba(6,182,212,0.35); border-radius:10px; width:44px; height:44px; text-align:center; vertical-align:middle; font-size:20px;">
                    🛡
                  </td>
        ...
        <div style="margin-top:28px;">
          <div style="color:#94a3b8; font-size:11px; letter-spacing:1.5px; text-transform:uppercase; margin-bottom:8px;">Network VAPT</div>
    ...
    <tr>
      <td style="background:#ffffff; padding:32px 40px; border:1px solid #e2e8f0; border-top:none;">
        ...
        <!-- Greeting -->
        <p style="color:#1e293b; font-size:15px; margin:0 0 8px;">Hi <strong>{{ $scan->user->name }}</strong>,</p>
        <p style="color:#475569; font-size:13px; line-height:1.7; margin:0 0 28px;">
          Your vulnerability scan of <strong style="font-family:monospace; color:#0f172a;">{{ $scan->target }}</strong>
          completed on <strong>{{ $scan->updated_at->format('F j, Y \a\t g:i A') }}</strong>.
          Please find your full report attached as a PDF.
          @if ($total === 0)
            <strong style="color:#16a34a;">No vulnerabilities were detected.</strong>
          @else
        ...
        @if ($total === 0)
          <!-- Clean scan -->
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#f0fdf4; border:1.5px solid #bbf7d0; border-radius:12px; padding:24px; margin-bottom:24px;">
            <tr>
              <td style="text-align:center; padding:20px;">
                <div style="font-size:32px; margin-bottom:8px;">✅</div>
                <div style="font-size:16px; font-weight:700; color:#16a34a;">All Clear</div>
                <div style="font-size:12px; color:#4ade80; margin-top:4px;">No vulnerabilities found on this target.</div>
              </td>
            </tr>
          </table>
        @else
          ...
          <div style="font-size:10px; text-transform:uppercase; letter-spacing:1px; color:#94a3b8; margin-bottom:12px; font-weight:600;">
            Top Findings{{ $vulns->count() > 5 ? ' (Top 5)' : '' }}
          </div>
          @foreach ($top as $v)
            @php
              $colors = ['critical'=>['bg'=>'#fee2e2','color'=>'#dc2626'],'high'=>['bg'=>'#ffedd5','color'=>'#ea580c'],'medium'=>['bg'=>'#fef3c7','color'=>'#d97706'],'low'=>['bg'=>'#e0f2fe','color'=>'#0284c7']];
              $col = $colors[$v->severity] ?? $colors['low'];
            @endphp
            <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e2e8f0; border-radius:8px; margin-bottom:8px; overflow:hidden;">
              <tr>
                ...
                <td width="70" align="right" style="padding:10px 14px;">
                  @if ($v->status === 'open')
          ...
          @if ($vulns->count() > 5)
            <p style="font-size:12px; color:#64748b; text-align:center; margin-top:8px;">
              + {{ $vulns->count() - 5 }} more findings in the attached PDF report.
            </p>
          @endif
        @endif
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:10px; padding:16px;">
          <tr>
            ...
            <td>
              <div style="font-size:13px; font-weight:700; color:#1e293b;">Full PDF Report Attached</div>
              <div style="font-size:12px; color:#64748b; margin-top:2px;">
                The attached PDF contains the complete findings with descriptions, CVE references, and remediation steps for every vulnerability.
              </div>
            </td>
          </tr>
        </table>
        ...
        <p style="font-size:12px; color:#94a3b8; margin-top:24px; line-height:1.6;">
          This report was automatically generated by ServerSentinel. If you have questions about these findings, please contact your security administrator.
        </p>
      </td>
    </tr>
        ...
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            <td style="font-size:11px; color:#94a3b8;">
              <strong style="color:#64748b;">ServerSentinel</strong> — Automated Security Platform
            </td>
